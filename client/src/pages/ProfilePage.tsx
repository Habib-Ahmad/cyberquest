		...
		<div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
			...
			<div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
				<Card className="p-4 text-center">
					<Flag className="h-8 w-8 text-primary-500 mx-auto mb-2" />
					<p
			...
			<Card className="p-6 mb-8">
				...
				<div
					className={`h-3 rounded-full overflow-hidden ${isDark ? 'bg-gray-800' : 'bg-gray-200'}`}
				>
					<div
			...
			<Card className="p-6">
				...
				{recentSolves.length > 0 ? (
					<div className="space-y-3">
						{recentSolves.map((solve) => (
							<Link
								...
								className={`flex items-center justify-between p-3 rounded-lg transition-colors group ${isDark ? 'bg-gray-800/50 hover:bg-gray-800' : 'bg-gray-50 hover:bg-gray-100'}`}
							>
								<div className="flex items-center gap-3">
									<div className="p-2 rounded-lg bg-primary-600/20 text-primary-600 group-hover:bg-primary-600 group-hover:text-white transition-colors">
										<CheckCircle className="h-4 w-4" />
									</div>
									<div>
										<p
											className={`font-medium group-hover:text-primary-600 transition-colors ${isDark ? 'text-white' : 'text-gray-900'}`}
										>
											{solve.challengeTitle}
										</p>
										<div
											className={`flex items-center gap-2 text-sm ${isDark ? 'text-gray-500' : 'text-gray-500'}`}
										>
					...
					<div className="text-center py-8">
						<Flag
							className={`h-12 w-12 mx-auto mb-3 ${isDark ? 'text-gray-700' : 'text-gray-300'}`}
						/>
						<p className={isDark ? 'text-gray-400' : 'text-gray-600'}>
